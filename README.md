...
//                                    dtype = GGUF type code (Q4_0/Q5_0/Q8_0/Q4_K/Q6_K).
...
that packed reconstruction is generalizing past the Metal/example corner into a CPU-agnostic library primitive. `nt_qmatvec(out, Wq, dtype, x, m, k)` keeps weights packed and dequantizes each block inline for the **whole common quant set on the CPU** — Q4_0, Q5_0, Q8_0, Q4_K, Q6_K — dispatched by GGUF dtype, with the Q6_K kernel lifted out of `infer_gguf_metal.c` into the library. each format is verified bit-close to `gguf_dequant → cblas` (relative error ~1e-6, `tests/test_qmatvec`). it means the CPU no longer has to blow Q4_0/Q8_0 up to f32 the way the f32 engines below do. the primitive is correctness-complete and single-threaded today; wiring the runners onto it (so a Q4_0 model stays packed end-to-end) and the speed path — threaded rows plus int8 activation-quant with SDOT/VNNI integer dot, the technique that makes llama.cpp and MNN fast — are in progress.
